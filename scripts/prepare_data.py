...
from handwriting_synthesis.config import ConfigValidationError, load_json_config, validate_prepare_config
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        help="Directory to save training and validation datasets"
    )
    parser.add_argument(
        "provider_name", type=str, nargs="?", default="",
        help="A short name used to lookup the corresponding factory class"
    )
    parser.add_argument(
    ...
    parser.add_argument("--config", type=str, default="", help="Path to prepare config JSON")
    ...
    if args.config:
        ...
        save_dir = cfg["output"]["prepared_data_dir"]
        provider_name = cfg["dataset"]["provider_name"]
        provider_args = [str(v) for v in cfg["dataset"]["provider_args"]]
    else:
        if not args.save_dir or not args.provider_name:
            parser.error("Either pass --config or provide positional args: save_dir provider_name [provider_args ...]")
    ...
    if max_len == 0:
        ...
        max_len = calculate_max_length(provider_class, *provider_args)
