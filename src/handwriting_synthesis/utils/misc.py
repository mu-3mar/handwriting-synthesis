...
from handwriting_synthesis.metrics import MovingAverage
from handwriting_synthesis.losses import BiVariateGaussian
import logging
import svgwrite
...
def plot_attention_weights(phi, seq, save_path='img.png', text='', thickness=10):
    ...
    for i, single_x in enumerate(x):
        temperatures = phi[i]
        denom = float(np.sum(temperatures))
        if not np.isfinite(denom) or denom <= 0:
            continue
        colors = np.asarray(temperatures, dtype=np.float64) / denom
        colors = np.nan_to_num(colors, nan=0.0, posinf=1.0, neginf=0.0)
        colors = np.clip(colors, 0.0, 1.0)
        axes[0].scatter(
            [single_x] * len(temperatures), y_ticks, c=colors, s=5, cmap='gray', vmin=0.0, vmax=1.0
        )
...
class HandwritingSynthesizer:
    ...
    def synthesize(self, c, output_path, show_attention=False, text='', thickness=10):
        try:
            ...
            logger.error(f'Error when synthesizing a handwriting: {e}')
