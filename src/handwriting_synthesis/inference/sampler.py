...
import os
import torch
from handwriting_synthesis.utils import visualize_strokes, plot_attention_weights
from handwriting_synthesis.data import transcriptions_to_tensor, Tokenizer
from handwriting_synthesis import models


def _model_state_finite(model):
    """Return False if any parameter contains nan or inf."""
    with torch.no_grad():
...
class UnconditionalSampler:
    ...
    def load_latest(cls, check_points_dir, device, bias=0):
        ...
        for dir_name in os.listdir(check_points_dir):
            ...
            if not matches:
                continue
            epoch_num = int(matches[0])
            candidates.append((epoch_num, os.path.join(check_points_dir, dir_name)))

        candidates.sort(key=lambda x: -x[0])

        for epoch_num, recent_checkpoint in candidates:
            try:
                sampler = cls.load(recent_checkpoint, device, bias)
            except Exception as e:
                print(f'Failed to load {recent_checkpoint}: {e}')
                continue
            if _model_state_finite(sampler.model):
                print(f'Loaded model weights from {recent_checkpoint} file')
                return sampler, epoch_num
            print(
                f'Skipping checkpoint with non-finite weights (corrupt/diverged): {recent_checkpoint}'
            )
