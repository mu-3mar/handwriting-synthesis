...
class BaseHandwritingTask(TrainingTask):
    def __init__(self, device, model, clip_values=None, learning_rate=5e-5, max_grad_norm=1.0):
        self._device = device
        self._model = model
        self._model = self._model.to(self._device)
        self._max_grad_norm = max_grad_norm
        self._optimizer = CustomRMSprop(
            self._model.parameters(), lr=learning_rate, alpha=0.95, eps=10 ** (-4),
    ...
    def train(self, batch):
        ...
        if not torch.isfinite(loss):
            logging.getLogger(__name__).error("Non-finite loss encountered before backward; skipping optimizer step.")
            return y_hat, torch.tensor(float("nan"), device=self._device)
        loss.backward()
        has_bad_grad = False
        for param in self._model.parameters():
            ...
            if grad is not None and not torch.isfinite(grad).all():
                ...
                break
        if has_bad_grad:
            logging.getLogger(__name__).error("Non-finite gradients detected; skipping optimizer step for this batch.")
            self._optimizer.zero_grad(set_to_none=True)
            return y_hat, torch.tensor(float("nan"), device=self._device)
        if self._max_grad_norm is not None and self._max_grad_norm > 0:
            nn.utils.clip_grad_norm_(self._model.parameters(), max_norm=self._max_grad_norm)
