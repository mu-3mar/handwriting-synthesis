...
from .step import TrainingTask
from handwriting_synthesis import utils
from handwriting_synthesis.utils import collate, compute_validation_loss, compute_validation_metrics
import logging
try:
        ...
        for epoch in range(initial_epoch, initial_epoch + epochs):
            ...
            if tqdm is not None:
                epoch_iter = enumerate(
                    tqdm(loader, total=num_batches, desc=f"Epoch {epoch}", leave=False, dynamic_ncols=True)
                )

            for i, data in epoch_iter:
                points, _ = data
                y_hat, loss = self._trainer.train(data)
                loss_value = float(loss.detach().item()) if torch.is_tensor(loss) else float(loss)
                if not math.isfinite(loss_value):
                    logger.error(
                        f"Skipping unstable batch at epoch={epoch} batch={i + 1}/{num_batches}: loss={loss_value}"
                    )
                    continue
